...
function FryPackingGame:init()
    super.init(self, _s("minigame_popup-fry_packing_game", "PACK!"), "full_layout", true)

    self.grid_size = 4 -- amount of cells in each row, as well as amount of rows.
...
function FryPackingGame:onStart()
    ...
    self.cone_front = self:spawnObject(Sprite("objects/frypackinggame/cone_front", 320, 720))
    self.cone_front:addFX(MaskFX(Game.battle.arena))
    self.cone_front:setOrigin(0.5, 1)
    self.cone_front:setScale(2)
    self.cone_front:setLayer(LIGHT_BATTLE_LAYERS["bullets"]+50)
    self.cone_back = self:spawnObject(Sprite("objects/frypackinggame/cone_back", 320, 720))
    self.cone_back:addFX(MaskFX(Game.battle.arena))
    self.cone_back:setOrigin(0.5, 1)
    self.cone_back:setScale(2)
    self.cone_back:setLayer(LIGHT_BATTLE_LAYERS["bullets"]-50)
    ...
    --Game.battle.soul.sprite:setSprite("player/pointer_up")
end
...
function FryPackingGame:update()
    ...
    if self.allPacked then
        if not self.show_score then
            ...
            self.cone_back:slideTo(320, 440, 1, "out-quad")
            self.cone_front:slideTo(320, 440, 1, "out-quad")
            Game.battle.timer:after(1, function () self:score(score) end)
    ...
    local grabbing = self.selected_bullet ~= nil
    if self._cwk_pointer_grabbing ~= grabbing then
        self._cwk_pointer_grabbing = grabbing
        Game.battle.soul.sprite:setSprite(grabbing and "player/pointer_up_2" or "player/pointer_up_1")
    end
...
function FryPackingGame:onEnd()
    Game.battle.soul.sprite:setSprite("player/heart")
