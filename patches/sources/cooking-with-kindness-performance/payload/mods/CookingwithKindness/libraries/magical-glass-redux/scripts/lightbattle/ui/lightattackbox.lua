...
function LightAttackBox:init(x, y)
    ...
    self.done = nil
    self._cwk_remove_scheduled = false
end
...
function LightAttackBox:createBolts()
    ...
    for i,battler in ipairs(Utils.shuffle(self.attackers)) do
        ...
        lane.score = 0
        lane.stretch = nil
        lane.direction = Game.battle.multi_mode and "left" or lane.weapon and lane.weapon.getLightBoltDirection and lane.weapon:getLightBoltDirection() or TableUtils.pick({"right", "left"})
        ...
        last_offset = TableUtils.pick{0, 10, 15} * 5
        local start_x
...
function LightAttackBox:update()
    ...
    if #self.lanes ~= 0 or #self.attackers == #Game.battle.auto_attackers then
        ...
        if Game.battle.cancel_attack or self.fading then
            ...
            if (self.target_sprite.scale_x < 0.08 or self.target_sprite.alpha < 0.08) and not self._cwk_remove_scheduled then
                self._cwk_remove_scheduled = true
