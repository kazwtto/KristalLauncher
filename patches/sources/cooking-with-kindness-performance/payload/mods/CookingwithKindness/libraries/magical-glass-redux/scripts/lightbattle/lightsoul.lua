...
function LightSoul:update()
    ...
    self._cwk_collided_bullets = self._cwk_collided_bullets or {}
    local collided_bullets = self._cwk_collided_bullets
    for i = #collided_bullets, 1, -1 do
        collided_bullets[i] = nil
    end
    Object.startCache()
    ...
    if self.inv_timer > 0 then
        ...
        if (amt % 2) == 1 then
            if self.sprite.color[1] ~= 0.5 or self.sprite.color[2] ~= 0.5 or self.sprite.color[3] ~= 0.5 then
                self.sprite:setColor(0.5, 0.5, 0.5)
            end
        else
            if self.sprite.color[1] ~= 1 or self.sprite.color[2] ~= 1 or self.sprite.color[3] ~= 1 then
