...
$packageOutput = Join-Path $patchesRoot "packages\cooking-with-kindness-mobile-performance-0.4.1.klpatch"
$temporaryArchive = "$packageOutput.tmp.zip"
$packageDirectory = Split-Path $packageOutput -Parent
if (-not (Test-Path -LiteralPath $packageDirectory)) { New-Item -ItemType Directory -Path $packageDirectory | Out-Null }
