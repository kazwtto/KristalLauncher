...
2. `GameScanner` fingerprints supported files by URI, size, and modification time. It reuses unchanged cache entries, including providers that do not expose reliable timestamps, parses changed packages with bounded concurrency, and publishes incremental results.
3. `LoveMetadataParser` accepts `.love`, `.zip`, and fused Windows `.exe` packages. It extracts size-limited Kristal `mod.json` metadata, ignores template placeholder titles such as `Example Mod`, `Exemple Mod`, and `Example Project`, and falls back to `conf.lua` or the package file name. Wrapper ZIPs may expose complete `.love` or fused `.exe` packages stored in nested folders; temporary scan copies remain inside the application cache.
...
5. `GameLauncher` copies the selected package into a content-addressed file under the app-private `staged/` directory. For fused executables, it locates the embedded ZIP payload in one pass; for wrapper ZIPs, it extracts only the selected nested package.
...
7. `GameLauncher` sets the embedded runtime's private static `gamePath` field and explicitly starts the non-exported `org.love2d.android.GameActivity` with an internal `file://` URI for the same staged path. Because the intent never leaves the application package, the runtime reads the private file directly instead of copying it into cache a second time. `KristalApplication` attaches `LoveOverlayManager` while that activity is visible.
...
| `GameAdapter` | Consistent list rendering, favorites, launch actions, and per-game patch settings |
...
| `AppearanceSettingsActivity` | Device-following, light, or dark theme selection |
...
Small preferences use Android `SharedPreferences`: selected folder URI, language, theme, favorites, recent games, global patch flags, per-game overrides, and update-check state. Cached game metadata and icons live in application-private files, while the last valid patch catalog uses the cache directory; both may be safely cleared through their owning flows. Installed external patches live under the application-private `files/patches/` directory.
...
- A game inside a wrapper ZIP is represented by the source document URI plus its safe archive entry path.
...
- Unit tests cover semantic version ordering, archive-path validation, template-title handling, fused-executable payload detection, and safe Lua-footer placement. Full ZIP rewrites, runtime shader behavior, and native activity launch still need broader automated and device coverage.
